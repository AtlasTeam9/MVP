...
vi.mock('../sessionHelpers', async (importOriginal) => {
    ...
    return {
        ...
        mapLoadSessionResponse: (...args) => mocks.mapLoadSessionResponse(...args),
    }
})
...
function setupSessionDefaults() {
    ...
    mocks.mapLoadSessionResponse.mockReset()
    mocks.mapLoadSessionResponse.mockImplementation((response = {}) => ({
        sessionId: response.session_id,
        device: mocks.createDeviceFromApiResponse(response.device || {}),
        ...
        results: mocks.mapResultsToRequirementResults(response.aggregate_results),
        isFinished: Boolean(response.is_finished),
    }))
}

function assertLoadSessionCall() {
...
function assertActiveSessionStoresUpdated({ mappedDevice, response, currentNode, mappedResults }) {
...
function createActiveSessionLoadResponse() {
    return {
...
function setupActiveLoadSessionMocks() {
    const mappedDevice = {
        id: 'mapped-device',
        assets: [{ id: 'asset-1' }],
    }
    ...
    const response = createActiveSessionLoadResponse()
    ...
    return { mappedDevice, mappedResults, currentNode, response }
}
...
describe('SessionService loadSessionFromFile active session', () => {
    ...
    it('loadSessionFromFile maps backend payload and updates all stores when session is active', async () => {
        const { mappedDevice, mappedResults, currentNode, response } = setupActiveLoadSessionMocks()

        const loaded = await SessionService.loadSessionFromFile({ name: 'saved-session.json' })

        assertLoadSessionCall()
        assertActiveSessionStoresUpdated({ mappedDevice, response, currentNode, mappedResults })
